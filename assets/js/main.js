...
(function () {
  ...
  /* ─────────────────────────────────────────────────────────
     Bodenaufbau-Animation im Hero
     Schaltet die Schichten der Reihe nach zu. Läuft nur, solange
     sie sichtbar ist, und steht still, wenn der Besucher in den
     Systemeinstellungen weniger Bewegung wünscht.
     ───────────────────────────────────────────────────────── */
  var buildup = $('#buildup');
  if (buildup) {
    ...
    // Dauer je Schritt; der Estrich braucht länger, das Ergebnis darf wirken.
    var HOLD = [1500, 1500, 1600, 1700, 2300, 3400];

    var calm = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    var stage = 0, timer = null, running = false;
    ...
    if (dotWrap) {
      caps.forEach(function (cap, i) {
        ...
        // Der Schrittname steckt schon in der Bildunterschrift — hier reicht er als Label.
        var name = cap.querySelector('b');
        b.setAttribute('aria-label', 'Schritt ' + (i + 1) + (name ? ': ' + name.textContent : ''));
    ...
    function show(n) {
      stage = n;
      buildup.setAttribute('data-stage', String(n));
    ...
    function tick() {
      show((stage + 1) % TOTAL);
      timer = window.setTimeout(tick, HOLD[stage]);
    }
    ...
    if (calm) {
      // Fertigen Aufbau zeigen, ohne Bewegung.
      show(TOTAL - 1);
    } else {
      show(0);
      if ('IntersectionObserver' in window) {
        ...
        }, { threshold: 0.25 }).observe(buildup);
      } else {
        start();
      }
